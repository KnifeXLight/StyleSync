...
def test_item_route_non_existing(logged_in_client):
    with logged_in_client:
        response = logged_in_client.get(
            '/views/item/999', follow_redirects=True)
        assert response.status_code == 200
...
def test_delete_item_route_non_existing(logged_in_client):
    with logged_in_client:
        response = logged_in_client.get(
            '/views/items/999', follow_redirects=True)
...
def test_delete_item_route_existing(logged_in_client, test_item):
    ...
    response = logged_in_client.get(
        f'/views/items/{item_id}', follow_redirects=True)
...
def test_filter_items(logged_in_client, app):
    with logged_in_client:
        with app.app_context():
            ...
            item = Item(name="Test Item", user_id=1,
                        image_url="test_image_url")
            db.session.add(item)
            db.session.commit()

            tag = Tag(item_id=item.id, filter_id=filter_.id,
                      category_id=category.id)
            db.session.add(tag)
            db.session.commit()

            response = logged_in_client.post(
                '/views/wardrobe/filter', data={'filter_id': filter_.id}, follow_redirects=True)
            assert response.status_code == 200
...
def test_filter_items_valid_filter(logged_in_client, app):
    with logged_in_client:
        with app.app_context():
            ...
            response = logged_in_client.post(
                '/views/wardrobe/filter', data={str(filter_.id): filter_.id}, follow_redirects=True)
            assert response.status_code == 200
            assert b'Test Item' in response.data
...
def test_filter_items_no_matching_filter(logged_in_client, app):
    with logged_in_client:
        with app.app_context():
            ...
            filter_ = Filter(name="Non-Matching Filter",
                             category_id=category.id)
            db.session.add(filter_)
            db.session.commit()
